...
const Library = {
  ...
  template: `
    <div class="library-roadmap">
      ...
      <div class="section" v-if="query" >
        <div class="library-search-banner">
          <span>Results for “{{ store.searchQuery.trim() }}”</span>
          <button class="library-search-clear" @click="store.searchQuery = ''">Clear search</button>
        </div>
      </div>

      <div class="section">
        <div class="lessons-grid roadmap-grid" v-if="filteredLessons.length">
          <div class="lesson-card roadmap-card" v-for="lesson in filteredLessons" :key="lesson.id" @click="startLesson(lesson)">
        ...
        <div v-else class="library-no-results">
          <div class="empty-icon" v-html="objectSvg"></div>
          <p>No lessons match “{{ store.searchQuery.trim() }}”.</p>
          <p class="empty-sub">Try a different word, or clear the search.</p>
        </div>
      </div>
      ...
      <div class="section saved-wip">
        ...
        <div v-if="filteredSaved.length > 0" class="library-grid saved-grid">
          <div v-for="item in filteredSaved" :key="item.name" class="library-item saved-item">
            ...
            <div class="remove-btn" @click.stop="removeSavedItem(item)">Remove</div>
          </div>
        </div>
        <div v-else-if="query" class="empty-library">
          <div class="empty-icon" v-html="objectSvg"></div>
          <p>No saved objects match “{{ store.searchQuery.trim() }}”.</p>
        </div>
  ...
  data() {
    return {
      // Shared search query written by the top bar; filters this page live.
      store: (window.CosmoKlub && window.CosmoKlub.store) || { searchQuery: '' },
      ranks: [
  ...
  computed: {
    // Normalised search term from the top bar.
    query() { return (this.store.searchQuery || '').trim().toLowerCase(); },
    // Lessons matching the query (title / description / rank). Empty query = all.
    filteredLessons() {
      const q = this.query;
      if (!q) return this.lessons;
      return this.lessons.filter((l) =>
        (l.title + ' ' + l.desc + ' ' + l.rank).toLowerCase().includes(q));
    },
    // Saved objects matching the query (name / type).
    filteredSaved() {
      const q = this.query;
      if (!q) return this.savedItems;
      return this.savedItems.filter((s) =>
        (s.name + ' ' + s.type).toLowerCase().includes(q));
    },
    hasResults() { return this.filteredLessons.length > 0 || this.filteredSaved.length > 0; },
  ...
  methods: {
    removeSavedItem(item) {
      const i = this.savedItems.indexOf(item);
      if (i !== -1) this.savedItems.splice(i, 1);
    },
    ...
    injectLibraryStyles() {
      ...
      style.textContent = `
        ...
        .library-search-banner { display: flex; align-items: center; justify-content: space-between; gap: 12px; background: rgba(124,58,237,0.1); border: 1px solid rgba(168,85,247,0.28); border-radius: 12px; padding: 10px 14px; color: var(--stardust); font-size: 13px; font-weight: 600; }
        .library-search-clear { background: none; border: 1px solid var(--border); color: var(--glow); font-family: var(--font); font-size: 12px; font-weight: 700; padding: 5px 12px; border-radius: 999px; cursor: pointer; transition: all var(--tr); }
        .library-search-clear:hover { border-color: var(--violet); background: rgba(168,85,247,0.14); }
        .library-no-results { display: flex; flex-direction: column; align-items: center; text-align: center; gap: 4px; padding: 32px 16px; color: var(--stardust); }
        .library-no-results .empty-icon { margin-bottom: 6px; opacity: 0.7; }
