...
// ── Graphing on-screen math keyboard (Desmos-style) ─────────────────────────
...
const GRAPH_KEY_SET = [
  { label: 'x', token: 'x' }, { label: 'y', token: 'y' }, { label: '(', token: '(' }, { label: ')', token: ')' }, { label: '⌫', action: 'backspace', tone: 'operator' },
  { label: '7' }, { label: '8' }, { label: '9' }, { label: '÷', token: '÷', tone: 'operator' }, { label: 'x²', token: '^2', tone: 'operator' },
  { label: '4' }, { label: '5' }, { label: '6' }, { label: '×', token: '×', tone: 'operator' }, { label: 'xʸ', token: '^', tone: 'operator' },
  { label: '1' }, { label: '2' }, { label: '3' }, { label: '−', token: '−', tone: 'operator' }, { label: '√', token: 'sqrt(', tone: 'operator' },
  { label: '0' }, { label: '.' }, { label: '=', token: '=' }, { label: '+', token: '+', tone: 'operator' }, { label: 'π', token: 'π', tone: 'operator' },
  { label: 'sin', token: 'sin(' }, { label: 'cos', token: 'cos(' }, { label: 'tan', token: 'tan(' }, { label: 'ln', token: 'ln(' }, { label: 'log', token: 'log(' },
  { label: ',', token: ',' }, { label: 'e', token: 'e' }, { label: '|x|', token: 'abs(' }, { label: 'a', token: 'a' }, { label: 'C', action: 'clear', tone: 'operator' },
];
...
const Planetarium = {
        ...
        <div class="graph-canvas-host" ref="graphHost">
          ...
          <div class="graph-kbd" v-if="activeRowId !== null" @pointerdown.prevent>
            <div class="graph-kbd-head">
              <span class="graph-kbd-title">Keyboard</span>
  ...
  computed: {
    ...
    graphKeys() { return GRAPH_KEY_SET; },
    live() {
  ...
  methods: {
    ...
    // TOUCH:  one finger → trace the point under it; two fingers → pan + pinch-zoom.
    ...
    onGraphDown(e) {
      ...
      if (this._pointers.size >= 2) {
        // Two fingers → pinch/pan; cancel any trace-in-progress.
        this._panning = false; this._tracing = false;
        this._beginPinch();
      } else if (touch) {
        // Single finger → trace under the finger.
        this._panning = false; this._pinch = null; this._tracing = true;
        const rect = c.getBoundingClientRect();
        this.updateTrace(e.clientX - rect.left, e.clientY - rect.top);
      } else {
        // Mouse → pan.
        this._tracing = false; this._pinch = null;
    ...
    onGraphMove(e) {
      ...
      if (this._tracing) {
        // Finger dragging → trace follows the finger along the nearest curve.
        this.updateTrace(e.clientX - rect.left, e.clientY - rect.top);
    ...
    onGraphUp(e) {
      ...
      if (this._pointers && this._pointers.size === 1) {
        // Dropped from two fingers to one → resume single-finger trace.
        this._pinch = null; this._panning = false; this._tracing = true;
        const rect = this.$refs.graphCanvas.getBoundingClientRect();
    ...
    injectStyles() {
      ...
      style.textContent = `
        ...
        /* On-screen math keyboard (graphing) — slides up from the bottom. */
        .graph-kbd { position: absolute; left: 0; right: 0; bottom: 0; z-index: 20; background: rgba(11, 9, 26, 0.97); border-top: 1px solid var(--border); backdrop-filter: blur(18px); padding: 10px 12px calc(12px + var(--safe-bot)); box-shadow: 0 -12px 40px rgba(4,2,16,0.5); animation: graphKbdUp 0.22s cubic-bezier(.4,0,.2,1); }
        @keyframes graphKbdUp { from { transform: translateY(100%); } to { transform: translateY(0); } }
        .graph-kbd-head { display: flex; align-items: center; justify-content: space-between; margin-bottom: 8px; }
        .graph-kbd-title { font-size: 11px; font-weight: 800; letter-spacing: 0.08em; text-transform: uppercase; color: var(--muted); }
        .graph-kbd-done { background: linear-gradient(135deg, var(--purple), var(--violet)); border: none; color: #fff; font-family: var(--font); font-size: 12.5px; font-weight: 700; padding: 6px 18px; border-radius: 999px; cursor: pointer; box-shadow: 0 2px 12px rgba(124,58,237,0.4); }
        .graph-kbd-grid { display: grid; grid-template-columns: repeat(5, minmax(0, 1fr)); gap: 7px; }
        .graph-kbd-key { display: flex; align-items: center; justify-content: center; min-height: 46px; border-radius: 11px; background: linear-gradient(180deg, #1c1638, #17122e); border: 1px solid rgba(124,58,237,0.16); color: var(--stardust); font-family: var(--font); font-size: 1rem; font-weight: 600; cursor: pointer; user-select: none; transition: transform var(--tr), background var(--tr), border-color var(--tr); }
