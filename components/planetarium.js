  ...
  methods: {
    ...
    injectStyles() {
      ...
      style.textContent = `
        ...
        .calc-sci { display: grid; grid-template-columns: minmax(0, 1fr) 320px; gap: 22px; align-items: stretch; }
        ...
        /* Tabs */
        .calc-tabs { display: flex; gap: 26px; border-bottom: 1px solid var(--border); }
        ...
        /* Keypad — auto-fills available height so it fits the screen */
        .calc-grid { display: grid; gap: 10px; flex: 1; }
        .calc-grid.cols-6 { grid-template-columns: repeat(6, minmax(0, 1fr)); grid-auto-rows: minmax(56px, 1fr); }
        .calc-grid.cols-4 { grid-template-columns: repeat(4, minmax(0, 1fr)); grid-auto-rows: minmax(56px, 1fr); }
        .calc-key { position: relative; display: flex; align-items: center; justify-content: center; min-height: 56px; padding: 4px; border-radius: 14px; background: linear-gradient(180deg, #1c1638, #17122e); border: 1px solid rgba(124,58,237,0.16); color: var(--stardust); font-family: var(--font); font-weight: 600; cursor: pointer; user-select: none; transition: transform var(--tr), background var(--tr), border-color var(--tr), box-shadow var(--tr); }
        ...
        @media (max-width: 900px) {
          /* Sidebar (history) moves above the keypad so the pad gets full width. */
          .calc-sci { grid-template-columns: 1fr; }
          .calc-main { order: 2; }
