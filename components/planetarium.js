...
const Planetarium = {
  ...
  methods: {
    ...
    // One finger / mouse  → pan (and tap-to-trace when it didn't move).
    // Two fingers         → pinch-zoom about the midpoint + pan by the midpoint.
    onGraphDown(e) {
      ...
      if (this._pointers.size === 1) {
        // Begin a pan gesture.
        this._panning = true; this._panMoved = false; this._pinch = null;
    ...
    onGraphUp(e) {
      ...
      // Tap (single pointer that never moved) → place a trace point.
      if (this._panning && !this._panMoved && (!this._pointers || this._pointers.size === 0)) {
        const rect = this.$refs.graphCanvas.getBoundingClientRect();
        this.updateTrace(e.clientX - rect.left, e.clientY - rect.top);
      }
      if ((this._panning && this._panMoved) || this._pinch) this.scheduleAnalysis();
      ...
      if (this._pointers && this._pointers.size === 1) {
        ...
        this._panning = true; this._panMoved = true;
        this._panStart = { x: pt.x, y: pt.y, view: { ...this.graph.view } };
      } else if (!this._pointers || this._pointers.size === 0) {
        this._panning = false; this._pinch = null;
      }
    },
    onGraphLeave() { if (!this._panning && !this._pinch && this.graph.trace) { this.graph.trace = null; this.scheduleDraw(); } },
    _beginPinch() {
    ...
    injectStyles() {
      ...
      style.textContent = `
        ...
        .graph-canvas-host { position: relative; height: calc(100dvh - var(--topbar-h) - var(--nav-h) - var(--safe-bot) - 92px); min-height: 420px; border: 1px solid var(--border); border-radius: var(--radius); overflow: hidden; background: rgba(8,6,22,0.55); }
        ...
        @media (max-width: 760px) { .calc-graph { grid-template-columns: 1fr; gap: 12px; } .graph-canvas-host { height: clamp(360px, 60vh, 620px); } }
