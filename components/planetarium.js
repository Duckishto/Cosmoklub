...
// ── Graphing on-screen math keyboard (Desmos-style), split into categories ───
// Shown only while a graph expression row is focused. `action` keys perform an
// edit (backspace / clear); everything else inserts its token at the caret.
const GRAPH_KEY_SETS = {
  basic: [
    { label: '7' }, { label: '8' }, { label: '9' }, { label: '÷', token: '÷', tone: 'operator' }, { label: 'x', token: 'x' },
    { label: '4' }, { label: '5' }, { label: '6' }, { label: '×', token: '×', tone: 'operator' }, { label: 'y', token: 'y' },
    { label: '1' }, { label: '2' }, { label: '3' }, { label: '−', token: '−', tone: 'operator' }, { label: 'x²', token: '^2', tone: 'operator' },
    { label: '0' }, { label: '.' }, { label: '(', token: '(' }, { label: ')', token: ')' }, { label: '+', token: '+', tone: 'operator' },
    { label: '=', token: '=' }, { label: 'xʸ', token: '^', tone: 'operator' }, { label: 'π', token: 'π' }, { label: 'e', token: 'e' }, { label: '|x|', token: 'abs(' },
  ],
  functions: [
    { label: '√', token: 'sqrt(' }, { label: '∛', token: 'cbrt(' }, { label: 'ln', token: 'ln(' }, { label: 'log', token: 'log(' }, { label: 'eˣ', token: 'exp(' },
    { label: '|x|', token: 'abs(' }, { label: 'π', token: 'π' }, { label: 'e', token: 'e' }, { label: 'a', token: 'a' }, { label: 'b', token: 'b' },
    { label: '(', token: '(' }, { label: ')', token: ')' }, { label: ',', token: ',' }, { label: 'floor', token: 'floor(' }, { label: 'round', token: 'round(' },
    { label: '=', token: '=' }, { label: '^', token: '^', tone: 'operator' }, { label: 'x', token: 'x' }, { label: '×', token: '×', tone: 'operator' }, { label: '÷', token: '÷', tone: 'operator' },
  ],
  trig: [
    { label: 'sin', token: 'sin(' }, { label: 'cos', token: 'cos(' }, { label: 'tan', token: 'tan(' }, { label: 'π', token: 'π' }, { label: 'x', token: 'x' },
    { label: 'sin⁻¹', token: 'asin(' }, { label: 'cos⁻¹', token: 'acos(' }, { label: 'tan⁻¹', token: 'atan(' }, { label: 'e', token: 'e' }, { label: 'y', token: 'y' },
    { label: 'sinh', token: 'sinh(' }, { label: 'cosh', token: 'cosh(' }, { label: 'tanh', token: 'tanh(' }, { label: '(', token: '(' }, { label: ')', token: ')' },
    { label: '=', token: '=' }, { label: '^', token: '^', tone: 'operator' }, { label: '×', token: '×', tone: 'operator' }, { label: '÷', token: '÷', tone: 'operator' }, { label: '−', token: '−', tone: 'operator' },
  ],
};
...
const Planetarium = {
    ...
    <div class="section calc-wrap" :class="{ 'calc-graphing': mode === 'graphing' }">
      ...
      <div v-show="mode === 'graphing'" class="calc-graph">
        <div class="graph-canvas-host" ref="graphHost">
          ...
          <div class="graph-panel">
            <div class="graph-rows">
              <div class="graph-row-wrap" v-for="row in graph.rows" :key="row.id">
                <div class="graph-row" :class="{ invalid: row.error, focused: activeRowId === row.id }">
                  ...
                  <!-- Read-only: typing happens via the on-screen keyboard only -->
                  <div class="graph-input" :class="{ empty: !row.text }" @click="focusRow(row)">{{ row.text }}<span class="graph-caret" v-if="activeRowId === row.id"></span></div>
                ...
                <div class="graph-slider" v-if="row.slider">
                  ...
                  <input type="range" class="graph-range" :min="row.slider.min" :max="row.slider.max" :step="row.slider.step" :value="row.slider.value" @input="onSlider(row, $event)" @pointerdown="pauseSlider(row)" />
          ...
          <!-- On-screen math keyboard: only visible while a row is focused -->
          <div class="graph-kbd" v-if="activeRowId !== null" @pointerdown.prevent>
            <div class="graph-kbd-head">
              <div class="graph-kbd-tabs">
                <button v-for="t in graphKbdTabs" :key="t.id" :class="{ active: graphKbdSet === t.id }" @click="graphKbdSet = t.id">{{ t.label }}</button>
              </div>
              <div class="graph-kbd-actions">
                <button class="graph-kbd-back" @click="graphKey({ action: 'clear' })" aria-label="Clear">C</button>
                <button class="graph-kbd-back" @click="graphKey({ action: 'backspace' })" aria-label="Backspace">⌫</button>
                <button class="graph-kbd-done" @click="blurRow">Done</button>
              </div>
            </div>
            <div class="graph-kbd-grid">
  ...
  data() {
    return {
      ...
      _histId: 0,
      activeRowId: null, // which graph row is being edited (null → keyboard hidden)
      graphKbdSet: 'basic', // active category on the graphing keyboard
      graphKbdTabs: [{ id: 'basic', label: 'Basic' }, { id: 'functions', label: 'Functions' }, { id: 'trig', label: 'Trig' }],
  ...
  computed: {
    ...
    graphKeys() { return GRAPH_KEY_SETS[this.graphKbdSet] || GRAPH_KEY_SETS.basic; },
    live() {
  ...
  watch: {
    mode(value) {
      this.applyGraphingChrome(value === 'graphing');
      if (value !== 'graphing') this.activeRowId = null; // close keyboard when leaving
  ...
  methods: {
    ...
    // The display line. While typing we show ONLY the expression (e.g. "9-9"),
    // never a live "= result". When the user presses "=", evaluate() collapses
    // `expression` to the answer (e.g. "81"), so we just render that — a clean,
    // solid result that also serves as the base for the next calculation. The
    // full "expression = result" record lives in History. Empty display shows 0.
    renderMath() {
    ...
    // ── Graphing: on-screen keyboard ──
    // Hides the app's bottom nav while graphing (so the graph gets the full
    // area, Desmos-style). Always reversible: leaving graphing or unmounting
    // removes the class, so the nav reliably slides back up.
    applyGraphingChrome(on) {
      const cls = 'graphing-active';
      if (on) document.body.classList.add(cls);
      else document.body.classList.remove(cls);
    },
    focusRow(row) { this.activeRowId = row.id; },
    blurRow() { this.activeRowId = null; },
    graphKey(k) {
      const row = this.graph.rows.find((r) => r.id === this.activeRowId);
      if (!row) return;
      if (k.action === 'backspace') {
        row.text = Array.from(row.text).slice(0, -1).join('');
      } else if (k.action === 'clear') {
        row.text = '';
      } else {
        row.text += (k.token != null ? k.token : k.label);
      }
      this.onRowInput(row);
    },
    addRow() {
      ...
      this.compileRow(row);
      this.activeRowId = row.id; // open the keyboard for the fresh row
      this.scheduleDraw();
    },
    removeRow(id) {
      this.graph.rows = this.graph.rows.filter((r) => r.id !== id);
      if (this._compiled) delete this._compiled[id];
      if (this.activeRowId === id) this.activeRowId = null;
      this.scheduleDraw(); this.scheduleAnalysis();
    },
    toggleRow(row) { row.visible = !row.visible; this.scheduleDraw(); this.scheduleAnalysis(); },
    onSlider(row, ev) {
      if (row.slider && ev && ev.target) row.slider.value = parseFloat(ev.target.value);
      this.scheduleDraw(); this.scheduleAnalysis();
    },
    // Grabbing the thumb stops the play animation so they don't fight over .value.
    pauseSlider(row) { if (row.slider && row.slider.playing) { row.slider.playing = false; } },
    togglePlay(row) {
    ...
    // ── Graphing: pointer interaction (Desmos-style) ──
    // TOUCH:  one finger → pan the view AND show the point under the finger;
    //         two fingers → pan + pinch-zoom.
    // MOUSE:  drag → pan; hover → trace; wheel → zoom.
    onGraphDown(e) {
      ...
      if (this._pointers.size >= 2) {
        // Two fingers → pinch/pan; drop the single-finger trace.
        this._panning = false; this._tracing = false;
        this._beginPinch();
      } else {
        // One finger (or mouse) → pan. On touch we also trace under the finger.
        this._pinch = null;
    ...
    onGraphMove(e) {
      if (this._pointers && this._pointers.has(e.pointerId)) {
        this._pointers.set(e.pointerId, { x: e.clientX, y: e.clientY });
      }
      // Pinch (two active pointers) takes priority.
      if (this._pinch && this._pointers && this._pointers.size >= 2) {
        this._updatePinch();
        return;
      }
      ...
      if (this._panning) {
        ...
        // On touch, keep the trace point under the finger while panning.
        if (this._tracing) this.updateTrace(e.clientX - rect.left, e.clientY - rect.top);
        else this.scheduleDraw();
      } else if (e.pointerType !== 'touch') {
        // Mouse hover → live trace.
        this.updateTrace(e.clientX - rect.left, e.clientY - rect.top);
      }
    },
    onGraphUp(e) {
      if (this._pointers) this._pointers.delete(e.pointerId);
      if ((this._panning && this._panMoved) || this._pinch || this._tracing) this.scheduleAnalysis();

      if (this._pointers && this._pointers.size === 1) {
        // Dropped from two fingers to one → resume single-finger pan + trace.
        this._pinch = null; this._panning = true; this._panMoved = true; this._tracing = true;
        const rect = this.$refs.graphCanvas.getBoundingClientRect();
        const [pt] = this._pointers.values();
        this._panStart = { x: pt.x, y: pt.y, view: { ...this.graph.view } };
        this.updateTrace(pt.x - rect.left, pt.y - rect.top);
      } else if (!this._pointers || this._pointers.size === 0) {
        this._panning = false; this._pinch = null; this._tracing = false;
      }
    },
    onGraphLeave() { if (!this._panning && !this._pinch && !this._tracing && this.graph.trace) { this.graph.trace = null; this.scheduleDraw(); } },
    _beginPinch() {
    ...
    _updatePinch() {
      ...
      const p = this._pinch, s = p.view;
      const rect = this.$refs.graphCanvas.getBoundingClientRect();

      // Graph coord under the pinch's original midpoint (anchor point that stays put).
      const ax = s.xmin + ((p.midX - rect.left) / this.gw) * (s.xmax - s.xmin);
      const ay = s.ymax - ((p.midY - rect.top) / this.gh) * (s.ymax - s.ymin);

      // Spreading fingers (dist grows) zooms in → smaller span → factor < 1.
      const factor = p.dist / dist;
      let view = {
        xmin: ax + (s.xmin - ax) * factor,
        xmax: ax + (s.xmax - ax) * factor,
        ymin: ay + (s.ymin - ay) * factor,
        ymax: ay + (s.ymax - ay) * factor,
      };
      // Also translate by however far the midpoint slid (two-finger drag).
      const dxPx = midX - p.midX, dyPx = midY - p.midY;
      const xShift = (dxPx / this.gw) * (view.xmax - view.xmin);
      const yShift = (dyPx / this.gh) * (view.ymax - view.ymin);
      this.graph.view = { xmin: view.xmin - xShift, xmax: view.xmax - xShift, ymin: view.ymin + yShift, ymax: view.ymax + yShift };
      this.scheduleDraw();
    },
    onGraphWheel(e) {
      e.preventDefault();
    ...
    injectStyles() {
      ...
      style.textContent = `
        ...
        .graph-row { display: flex; align-items: center; gap: 8px; background: rgba(255,255,255,0.04); border: 1px solid var(--border); border-radius: 10px; padding: 8px 10px; transition: border-color var(--tr); }
        .graph-row.focused { border-color: var(--violet); box-shadow: 0 0 0 1px rgba(168,85,247,0.35); }
        ...
        .graph-input { flex: 1; min-width: 0; display: flex; align-items: center; color: var(--stardust); font-family: var(--font); font-size: 15px; padding: 3px 0; min-height: 22px; cursor: text; white-space: nowrap; overflow-x: auto; }
        .graph-input.empty { color: var(--muted); }
        .graph-caret { display: inline-block; width: 2px; height: 1.05em; margin-left: 1px; background: var(--violet); animation: graphCaretBlink 1s steps(1) infinite; }
        @keyframes graphCaretBlink { 0%, 50% { opacity: 1; } 50.01%, 100% { opacity: 0; } }
        ...
        .graph-range { flex: 1; min-width: 0; accent-color: var(--violet); cursor: pointer; touch-action: none; }
        ...
        .graph-controls button { width: 34px; height: 34px; display: flex; align-items: center; justify-content: center; background: none; border: none; border-radius: 7px; color: var(--stardust); font-size: 17px; line-height: 1; cursor: pointer; transition: background var(--tr), color var(--tr); }
        ...
        .graph-trace-chip { position: absolute; bottom: 14px; left: 14px; z-index: 5; display: flex; align-items: center; gap: 7px; background: rgba(13,11,30,0.9); border: 1px solid var(--border); border-radius: 8px; padding: 6px 12px; font-family: var(--font); font-size: 13px; font-weight: 600; color: var(--stardust); backdrop-filter: blur(6px); }
        .graph-trace-dot { width: 9px; height: 9px; border-radius: 50%; }

        /* On-screen math keyboard (graphing) — slides up, with category tabs. */
        .graph-kbd { position: absolute; left: 0; right: 0; bottom: 0; z-index: 20; background: rgba(11, 9, 26, 0.97); border-top: 1px solid var(--border); backdrop-filter: blur(18px); padding: 9px 12px calc(11px + var(--safe-bot)); box-shadow: 0 -12px 40px rgba(4,2,16,0.5); animation: graphKbdUp 0.22s cubic-bezier(.4,0,.2,1); }
        @keyframes graphKbdUp { from { transform: translateY(100%); } to { transform: translateY(0); } }
        .graph-kbd-head { display: flex; align-items: center; justify-content: space-between; gap: 10px; margin-bottom: 8px; }
        .graph-kbd-tabs { display: flex; gap: 4px; background: rgba(255,255,255,0.04); border: 1px solid var(--border); border-radius: 999px; padding: 3px; }
        .graph-kbd-tabs button { border: none; background: none; cursor: pointer; color: var(--muted); font-family: var(--font); font-size: 12px; font-weight: 700; padding: 6px 16px; border-radius: 999px; transition: color var(--tr), background var(--tr); }
        .graph-kbd-tabs button.active { color: #fff; background: linear-gradient(135deg, var(--purple), var(--violet)); box-shadow: 0 2px 10px rgba(124,58,237,0.4); }
        .graph-kbd-actions { display: flex; align-items: center; gap: 8px; }
        .graph-kbd-back { width: 42px; height: 34px; display: flex; align-items: center; justify-content: center; background: linear-gradient(180deg, #2f2361, #271c50); border: 1px solid rgba(168,85,247,0.34); color: var(--glow); font-size: 16px; border-radius: 10px; cursor: pointer; transition: background var(--tr); }
        .graph-kbd-back:hover { background: linear-gradient(180deg, #3a2b76, #2f2361); }
        .graph-kbd-done { background: linear-gradient(135deg, var(--purple), var(--violet)); border: none; color: #fff; font-family: var(--font); font-size: 12.5px; font-weight: 700; padding: 8px 18px; border-radius: 999px; cursor: pointer; box-shadow: 0 2px 12px rgba(124,58,237,0.4); }
        .graph-kbd-grid { display: grid; grid-template-columns: repeat(5, minmax(0, 1fr)); gap: 7px; }
        .graph-kbd-key { display: flex; align-items: center; justify-content: center; min-height: 44px; border-radius: 11px; background: linear-gradient(180deg, #1c1638, #17122e); border: 1px solid rgba(124,58,237,0.16); color: var(--stardust); font-family: var(--font); font-size: 1rem; font-weight: 600; cursor: pointer; user-select: none; transition: transform var(--tr), background var(--tr), border-color var(--tr); }
        .graph-kbd-key:hover { background: linear-gradient(180deg, #271d4c, #201741); border-color: rgba(168,85,247,0.5); }
        .graph-kbd-key:active { transform: translateY(1px); }
  ...
  beforeUnmount() {
    this.applyGraphingChrome(false); // safety: never leave the nav hidden
