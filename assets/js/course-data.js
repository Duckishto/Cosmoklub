...
const getBlueprintLessons=categoryId=>{
  return COURSE_BLUEPRINTS[categoryId].sections.flatMap(
...
const rotateAnswers=(answers,correctIndex,shift)=>{
  ...
  const rotated=answers
    .slice(amount)
    .concat(answers.slice(0,amount));

  const correctAnswer=
    (correctIndex-amount+count)%count;
...
const uniqueOptions=(correct,candidates,limit=4)=>{
  const values=[
    correct,
    ...candidates.filter(
...
// Harder stages use larger answer sets.
// Foundation keeps four choices.
// Intermediate uses five.
// Advanced and Expert use six.
const getAnswerChoiceCount=sectionIndex=>{
  return DIFFICULTY_STAGES[sectionIndex]?.answerChoices||4;
};
...
const getDistractorFacts=(categoryId,number)=>{
  ...
  const indexes=[
    ...
    value=>(value-1)%lessons.length
  );

  return indexes.map(
    index=>lessons[index][3]
  );
};
...
// Reusable scientifically plausible distractors.
//
// These are intentionally more convincing than obviously
// incorrect answers. Harder lessons therefore require the
// student to understand the actual physical mechanism instead
// of simply spotting one correct-looking keyword.
const CATEGORY_DISTRACTORS={
  stars:[
    'The observed property is controlled only by surface color and is independent of mass, composition, and evolutionary state.',

    'The phenomenon can be explained entirely by orbital distance from Earth without considering stellar physics.',

    'All stars of the same age must have identical luminosity, temperature, and radius.',

    'The strongest interpretation is the one based on a single image even when spectroscopy and time-series data disagree.',

    'The process requires no energy transport between the stellar interior and surface.'
  ],

  galaxies:[
    'Visible starlight traces essentially all of the galaxy\'s gravitating mass at every radius.',

    'Galaxy morphology alone uniquely determines the system\'s age and future evolution.',

    'Direct collisions between individual stars dominate the dynamics of most galaxy mergers.',

    'Gas, dust, dark matter, and environment can be ignored when interpreting a galaxy image.',

    'A single optical wavelength provides a complete inventory of a galaxy\'s physical components.'
  ],

  cosmology:[
    'The observation can be interpreted accurately without accounting for light-travel time or cosmic expansion.',

    'Every redshift can be treated as an ordinary local Doppler velocity using the same approximation at any distance.',

    'The observable universe is necessarily identical to the entire universe.',

    'One cosmological measurement by itself fixes every parameter of the universe without degeneracy.',

    'Gravitationally bound systems expand internally at exactly the same rate as the large-scale universe.'
  ],

  planets:[
    'Orbital distance alone determines a planet\'s atmosphere, surface state, and habitability.',

    'Planets with similar radii must have the same bulk composition and internal structure.',

    'A strong magnetic field is guaranteed whenever a planet contains an iron-rich core.',

    'A planet inside the traditional habitable zone is automatically habitable.',

    'Present-day appearance is sufficient to reconstruct a planet\'s full formation and geological history.'
  ],

  nebulae:[
    'All nebulae share the same origin and differ mainly because of viewing angle.',

    'Visible color alone is sufficient to identify the dominant physical process in every nebula.',

    'Dust affects only brightness and never changes the observed spectrum or wavelength dependence.',

    'A cloud that appears dark must be physically colder than every bright nebula in the same image.',

    'Multiwavelength observations add little because all wavelengths trace the same gas and energy processes.'
  ],

  observing:[
    'Increasing magnification always reveals additional real detail regardless of aperture, seeing, or target brightness.',

    'A brighter integrated magnitude guarantees that an extended object will be easier to see.',

    'Atmospheric transparency and seeing describe the same observing limitation and can be treated interchangeably.',

    'A telescope\'s focal length alone determines both its light-gathering power and diffraction-limited resolution.',

    'Longer exposure time removes the need for accurate tracking, calibration, and stable optics.'
  ]
};
...
const fillOptions=(
  ...
  const count=
    getAnswerChoiceCount(sectionIndex);

  const extras=
    CATEGORY_DISTRACTORS[categoryId]||[];
...
const makeLessonQuestions=(
  ...
  const difficulty=
    DIFFICULTY_STAGES[sectionIndex];

  const factDistractors=
    getDistractorFacts(
      categoryId,
      number
    );

  const depthDistractors=
    getDistractorDepth(
      categoryId,
      number
    );
  ...
  // ---------------------------------------------------------
  // QUESTION 3
  // True / false concept discrimination.
  // ---------------------------------------------------------
  ...
  // ---------------------------------------------------------
  // QUESTION 4
  // Observation-to-concept matching.
  // ---------------------------------------------------------
  ...
  // ---------------------------------------------------------
  // QUESTION 6+
  // Scientific-method questions begin here.
  // ---------------------------------------------------------
  ...
  // QUESTIONS 9–15
  //
  // These progressively shift away from direct recall.
  //
  // Intermediate:
  //   evidence discrimination and competing models.
  //
  // Advanced:
  //   conflicting observations and model testing.
  //
  // Expert:
  //   uncertainty, domains of validity and experimental design.
  // ---------------------------------------------------------

  if(difficulty.lessonQuestions>=9){
    const correct=
      `The observation should be interpreted by connecting the measured evidence to this lesson's mechanism: ${depth}`;

    questions.push(
      makeQuestion(
        `${categoryId}-${number}-q9`,
        'multiple-choice',

        `A new dataset is broadly consistent with ${title}, but one measurement is ambiguous. Which interpretation is the strongest?`,

        fillOptions(
          correct,

          [
            `Accept the interpretation immediately because one measurement has the expected sign.`,

            `Reject the interpretation because every measurement must be perfectly identical to the simplified model.`,

            `Choose whichever explanation uses the most technical terminology.`,
        ...
        `Strong inference combines the complete pattern of evidence with an appropriate physical mechanism and its uncertainties. ${depth}`,
  ...
  if(difficulty.lessonQuestions>=10){
    const correct=
      `Identify which observable would change if the mechanism behind ${title} were different, then measure that observable independently.`;

    questions.push(
      makeQuestion(
        `${categoryId}-${number}-q10`,
        'multiple-choice',

        `Two competing models both reproduce the most obvious feature of ${title}. What is the best next step?`,

        fillOptions(
          correct,

          [
            `Prefer the older model because it has been used for longer.`,

            `Average the two explanations even if they make incompatible physical predictions.`,

            `Use the same original observation repeatedly without collecting a new constraint.`,
        ...
        `A discriminating observation is one for which competing models predict measurably different outcomes. ${CATEGORY_EVIDENCE[categoryId]}`,
  ...
  if(difficulty.lessonQuestions>=11){
    const correct=
      `The apparent contradiction may be produced by different scales, wavelengths, environments, or selection effects, so the measurements should be placed in the same physical context before comparing them.`;

    questions.push(
      makeQuestion(
        `${categoryId}-${number}-q11`,
        'multiple-choice',

        `Two studies of ${title} report apparently different results. Which response demonstrates the strongest scientific reasoning?`,

        fillOptions(
          correct,

          [
            `Assume one study must be fraudulent because scientific measurements should never differ.`,

            `Keep only the result with the smaller numerical uncertainty even if it measures a different physical quantity.`,

            `Treat the disagreement as proof that the underlying physics is unknowable.`,
        ...
        `Before declaring a conflict, compare what was measured, how it was measured, and under which assumptions. ${CATEGORY_APPLICATION[categoryId]}`,
  ...
  if(difficulty.lessonQuestions>=12){
    const correct=
      `A valid explanation should reproduce the defining observation, remain consistent with ${fact.toLowerCase()}, and also agree with independent evidence described elsewhere in the category.`;

    questions.push(
      makeQuestion(
        `${categoryId}-${number}-q12`,
        'multiple-choice',

        `Which criterion would provide the strongest test of a physical model for ${title}?`,

        fillOptions(
          correct,

          [
            `The model should contain the largest number of adjustable parameters so that any result can be fitted.`,

            `The model only needs to explain the single image shown in the lesson.`,

            `The model is acceptable whenever its conclusion sounds similar to the lesson title.`,
        ...
        `Useful models make constrained predictions and must agree with more than one independent observation. ${CATEGORY_EVIDENCE[categoryId]}`,
  ...
  if(difficulty.lessonQuestions>=13){
    const correct=
      `Separate the direct measurement from the inferred physical quantity, propagate the uncertainty, and test whether alternative mechanisms could produce the same signal.`;

    questions.push(
      makeQuestion(
        `${categoryId}-${number}-q13`,
        'multiple-choice',

        `An astronomer claims that one observed signal proves a particular interpretation of ${title}. Which critique is most appropriate?`,

        fillOptions(
          correct,

          [
            `Any numerical measurement automatically proves the interpretation attached to it.`,

            `Alternative explanations can be ignored if the preferred model is common in textbooks.`,

            `Uncertainty matters only when the measurement is made from Earth rather than from space.`,
        ...
        `Observation and interpretation are not the same step. Expert reasoning asks how uniquely the measured signal maps onto the proposed physics.`,
  ...
  if(difficulty.lessonQuestions>=14){
    const correct=
      `Use the simplified model only within the regime where its assumptions remain valid, and switch to a more complete treatment when the system leaves that regime.`;

    questions.push(
      makeQuestion(
        `${categoryId}-${number}-q14`,
        'multiple-choice',

        `A simple model of ${title} works well for ordinary cases but fails for an extreme object. What is the most defensible conclusion?`,

        fillOptions(
          correct,

          [
            `The extreme object must be impossible because the simple model did not predict it.`,

            `The simple model should be forced to fit by ignoring the measurements that disagree.`,

            `All previous results obtained with the model are automatically invalid.`,
        ...
        `Approximations have domains of validity. Failure outside that domain does not erase their usefulness inside it, but it signals the need for additional physics.`,
  ...
  if(difficulty.lessonQuestions>=15){
    const correct=
      `Design an observation that makes a quantitative prediction from the mechanism, measure it with an independent method, and compare the result with plausible alternatives.`;

    questions.push(
      makeQuestion(
        `${categoryId}-${number}-q15`,
        'multiple-choice',

        `You are asked to design an expert-level investigation of ${title}. Which plan is scientifically strongest?`,

        fillOptions(
          correct,

          [
            `Collect as many images as possible without defining a prediction or comparison model.`,

            `Measure only the variable that most strongly supports the preferred explanation.`,

            `Use a single instrument so that no cross-check between methods is possible.`,
        ...
        `The strongest investigation links a physical model to a testable prediction and checks it with independent evidence. ${CATEGORY_EVIDENCE[categoryId]}`,
  ...
  // ---------------------------------------------------------
  // Topic-specific calculation / application questions.
  // ---------------------------------------------------------
  ...
  if(special){
    const slot=
      Math.min(
        ...
        Math.max(
          4,
          difficulty.lessonQuestions-1
        )
      );

    const correct=
      special.answers[
        special.correctAnswer
      ];
    ...
    const expandedAnswers=
      fillOptions(
        ...
        special.answers.filter(
          (_,index)=>
            index!==special.correctAnswer
        ),
...
const makeActivities=(
  ...
  if(
    ...
    activities.push({
      ...
      question:
        'Put these stages of star formation in the correct physical order.',
      ...
      explanation:
        'Gravity drives the initial collapse. A protostar forms before core hydrogen fusion becomes self-sustaining; the onset of stable fusion marks arrival on the main sequence.'
    });
  }
  ...
  if(
    ...
    activities.push({
      ...
      question:
        'Arrange these broad cosmic stages from earliest to latest.',
      ...
      explanation:
        'As expansion cooled the universe, nucleosynthesis occurred first, neutral atoms formed later at recombination, then the dark ages ended as the first luminous structures appeared.'
    });
  }
  ...
  if(
    ...
    activities.push({
      ...
      question:
        'Arrange the simplified stages of rocky-planet growth.',
      ...
      explanation:
        'Planet formation is not perfectly linear, but growth generally proceeds from small solids to gravitationally interacting planetesimals and embryos before final assembly.'
    });
  }

  if(
    categoryId==='observing'&&
    number===16
  ){
    activities.push({
      ...
      question:
        'Put this simplified imaging workflow in a sensible order.',
      ...
      explanation:
        'Good processing cannot replace poor acquisition. Planning, tracking, calibration, registration, stacking, and controlled processing form one connected workflow.'
    });
  }
...
const buildTeachingSections=(
  ...
  const categoryTitle=
    COURSE_BLUEPRINTS[categoryId].title;
  ...
  const sections=[
    {
      ...
      text:
        `${description} Before focusing on the details, place ${title.toLowerCase()} inside the broader subject of ${categoryTitle.toLowerCase()}. ${CATEGORY_FOUNDATIONS[categoryId]}`
    },
...

    {
      ...
      text:
        `When reasoning about ${title.toLowerCase()}, avoid thinking in terms of only one visible property. ${CATEGORY_VARIABLES[categoryId]} Which of these variables matters most depends on the specific object and observation.`
    },

    {
      ...
      text:
        `Astronomers do not identify ${title.toLowerCase()} from definitions alone. ${CATEGORY_EVIDENCE[categoryId]} Measurements are compared with physical models to determine which interpretation is most consistent with the evidence.`
    },

    {
      ...
      text:
        `Ask what causes the behavior instead of memorizing only what happens. For ${title.toLowerCase()}, connect the observed result to the physical mechanism described earlier. ${CATEGORY_APPLICATION[categoryId]}`
    },
  ...
  if(sectionIndex>=1){
    sections.push({
      ...
      text:
        `At the ${difficulty.name.toLowerCase()} level, compare ${title.toLowerCase()} with nearby concepts rather than treating it in isolation. Ask which observations are shared, which mechanisms differ, and what measurement would allow an astronomer to distinguish between competing explanations.`
    });
  }
  ...
  if(sectionIndex>=2){
    sections.push({
      ...
      text:
        `${CATEGORY_CONNECTIONS[categoryId]} Understanding ${title.toLowerCase()} therefore improves your ability to reason about topics outside this individual lesson.`
    });
  }
  ...
  if(sectionIndex>=3){
    sections.push({
      ...
      text:
        `Scientific models simplify reality. When applying a model to ${title.toLowerCase()}, identify its assumptions and the range where it is valid. A useful approximation can still become inaccurate when conditions, scales, velocities, densities, wavelengths, or environments move outside the range for which the approximation was designed.`
    });
  }
  ...
  if(sectionIndex>=3){
    sections.push({
      ...
      text:
        `At expert level, separate observation from interpretation. First state what was actually measured. Then identify the physical inference. Finally ask what alternative process could produce a similar signal and what additional observation would discriminate between the possibilities.`
    });
  }
  ...
  sections.push({
    ...
    text:
      `NASA missions, observatories, spacecraft, and scientific archives provide observations related to ${title.toLowerCase()}. The lesson searches NASA's image archive using the topic "${nasaSearch}", giving you a real observational example before the practice section.`
  });
...
const makeLesson=(
  ...
  const depth=
    LESSON_DEPTH[categoryId][number-1]
    ||fact;
  ...
  return{
    ...
    content:{
      intro:
        `${description} This ${difficulty.name.toLowerCase()} lesson teaches the concept in depth before the practice section begins.`,
...
const buildQuizQuestions=(
  ...
  sourceLessons.forEach(
    lesson=>{
      ...
      if(isFinal){
        // The final heavily favors the highest-order
        // questions while retaining earlier conceptual
        // questions for broad coverage.

        const hardStart=
          Math.max(
            0,
            Math.floor(
              questions.length*0.45
            )
          );
        ...
        // Later section quizzes start deeper inside
        // each lesson's question set.
        //
        // This makes ADVANCED and EXPERT quizzes
        // contain proportionally more interpretation,
...
const makeQuiz=(
  ...
  return{
    ...
    nasaSearch:
      `${COURSE_BLUEPRINTS[categoryId].title} NASA`,

    content:{
      intro:
        isFinal
          ?`This 40-question final challenge samples concepts from all four sections. Expect application, comparison, evidence evaluation, model limitations, interpretation, and quantitative reasoning where appropriate.`

          :`This ${difficulty.name.toLowerCase()} quiz tests more than recall. Later stages use five or six plausible choices, and incorrect options may be scientifically true statements that belong to a different concept or rely on an invalid assumption.`,
      ...
      sections:
        isFinal
          ?[
            {
              title:
                'Final Review: Core Principles',

              text:
                `Review the major physical relationships from the complete ${COURSE_BLUEPRINTS[categoryId].title} category before attempting the final challenge. Focus on cause and effect instead of isolated definitions.`
            },
            ...
            {
              title:
                'Final Challenge Strategy',

              text:
                'Use relationships between concepts, eliminate answers that are true but irrelevant, identify hidden assumptions, check units in numerical questions, and prefer explanations that survive several independent observational constraints.'
            }
          ]
          ...
          :[
            {
              ...
              text:
                `Before beginning the quiz, mentally connect the lessons in this section. Identify which ideas describe observations, which describe underlying mechanisms, and which variables determine the outcome.`
            },
...

            {
              ...
              text:
                'Do not choose an answer merely because it sounds scientific. Identify exactly what the question asks, then connect the observation to the relevant physical mechanism.'
            }
          ],
  ...
  (
    ...
    category.sections.forEach(
      (
        ...
        sections.push({
          id:
            `${categoryId}-section-${sectionIndex+1}`,
